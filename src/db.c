...
{
#define Q_TMPL "SELECT f.id FROM files f WHERE f.path = '%q' AND f.subtrack = 1 ORDER BY sample_offset LIMIT 1 OFFSET %d;"
  char *query;
  int ret;

  query = sqlite3_mprintf(Q_TMPL, path, subtrack);
  if (!query)
    {
      DPRINTF(E_LOG, L_DB, "Out of memory for query string\n");
